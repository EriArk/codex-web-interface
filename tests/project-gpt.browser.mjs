...
try {
  ...
  await popup.getByLabel("Настройки GPT проекта", { exact: true }).click();
  await popup.getByRole("button", { name: "Поведение ИИ", exact: true }).click();
  const profile = page.getByRole("dialog", { name: "Поведение ИИ проекта", exact: true });
  await profile.getByText("Дополнительные пожелания проекта", { exact: true }).click();
  await profile.getByLabel("Запускать подходящие тесты и сборку", { exact: true }).check();
  await profile.getByLabel("Свои правила проекта").fill("Проверять API World.");
  await profile.getByRole("button", { name: "Сравнить изменения" }).click();
  await profile.getByRole("button", { name: "Применить профиль" }).click();
  await expect(profile.getByRole("status")).toContainText("Профиль сохранён");
