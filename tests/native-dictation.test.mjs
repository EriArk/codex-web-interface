...
test('native dictation uses the consumer transcription route only and projects only text',async()=>{
  ...
  globalThis.fetch=async(path,options)=>{
    ...
    if(path==='/api/auth/session')return Response.json({accessToken:'fixture-private-token'});
    assert.equal(path,'/backend-api/transcribe');
    assert.deepEqual([...options.body.keys()],['file']);
    const file=options.body.get('file');assert.equal(file.type,'audio/wav');assert.equal(file.name,'dictation.wav');
  ...
  try{
    const page={url:()=> 'https://chatgpt.com/c/original',evaluate:(fn,input)=>fn(input)};
    const result=await transcribeDictation(page,{audio:Buffer.from('audio fixture').toString('base64'),mime:'audio/wav'});
    assert.deepEqual(result,{status:200,text:'Проверка диктовки.'});assert.equal(calls.length,2);
  }finally{globalThis.fetch=original;}
});
test('native dictation rejects unrecognized inputs and never requests alternate origins or billing models',async()=>{
  const page={url:()=> 'https://chatgpt.com/',evaluate:()=>{throw Error('Unexpected native call');}};
  for(const input of [{audio:'YQ==',mime:'__proto__'},{audio:'YQ==',mime:'audio/wav',url:'https://other.invalid'}, {audio:'!',mime:'audio/wav'}])
...
test('login failures and native refusal preserve failure without fallback',async()=>{
  ...
  try{
    ...
    globalThis.fetch=async path=>path==='/api/auth/session'?Response.json({accessToken:'private'}):new Response('refused',{status:403});
    assert.deepEqual(await transcribeDictation(page,input),{status:403,error:'DICTATION_NATIVE_FAILED'});
  }finally{globalThis.fetch=original;}
});
