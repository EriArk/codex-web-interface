...
Final follow-up: **526/526 tests** and lint passed after the socket fix. Exact-source image `60b326892` passed first-owner startup, `owner-team-check`, legacy-cookie continuity, login `eriark` with the old fixture password, ordinary private project reads, shared-project access and backend registration pause. Separate gateway and engine restarts preserved access. Restoring the original fixture configuration/database and restarting actual previous image `379fa17` restored ordinary personal access with the same original session/password. These disposable image checks made no native connections. Live read-only Doctor independently confirmed production SSH/Companion/Codex/GPT health; the fixed Windows GitHub worker confirmed the existing identity without a repository mutation.

The verified activation is queued, not yet installed at this checkpoint. `codex-web-owner-team-60b326892.service` waits under the native-work maintenance guard. The old engine/gateway were healthy on the final inspection; their current active/unknown Codex and GPT work was left intact. Installation receipts and protected verification evidence are retained in the installation state directory.
...
Release follow-up: exact-source image `99953827f` passed private-socket/schema-28 startup, public auth/CSP boundary, PWA publication, independent gateway restart and engine reconnect checks in disposable network-isolated containers. Live read-only Doctor confirmed SSH, Companion stdio, Codex login/models/planning, public health and GPT health. The protected verification receipt and logs are saved with the installation. The guarded personal-stage installer is running in **waiting** state; its latest check showed two Codex work/unknown blockers and a healthy old release. This records a queued update, not a completed cutover.
