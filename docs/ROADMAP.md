...
5. **Intake and Issue Drawer (#210/#211).** The minimal common conversation binding model (#209) and existing Project GPT integration are implemented: [scope, migration, checks and remaining role integrations](CONVERSATION_BINDINGS.md). Next implement Intake, then Issue Drawer. Preserve explicit handoff, reviewed publication and per-item receipts. This foundation does not complete all roles in #209; release installation is separately guarded.
