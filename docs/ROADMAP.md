...
- Hub engine `be0759f` is installed; the guarded updater completed successfully, verified on 23 September. The previous installed Doctor and owner-side Activity/GitHub/Project GPT/Intake reads passed; the actual Windows GitHub worker hash matches. Everyday Issue Drawer verification additionally fixes recollection after removal without replaying a lost acknowledgement; that follow-up is a separately guarded release. Owner native GPT was last upgraded to `26.915.31945-audit-e744924`; its real models/catalog/history reads passed. The separate member native runtime was not upgraded in that pass. The [GPT audit](GPT_STABILITY_AUDIT_2026-09-23.md) records checks and limits.
