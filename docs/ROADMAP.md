...
5. **Intake and Issue Drawer (#210/#211).** The minimal common conversation binding model (#209), Project GPT integration and persistent read-only Project Intake are implemented: [bindings](CONVERSATION_BINDINGS.md), [Intake flow, recovery and checks](PROJECT_INTAKE.md). Next: Issue Drawer with reviewed publication and per-item receipts. This does not complete all remaining roles in #209; release installation is separately guarded.
