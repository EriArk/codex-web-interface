...
The viewer/editor flow now includes non-writing draft preview/download and close/conflict/line-ending recovery. Multi-file uploads add progress/cancellation, explicit collision choices and durable recovery. The following web-only pass adds multiple selection and reviewed batch copy/move/delete, with per-entry receipts and partial-progress recovery. See [file editor flow](FILE_EDITOR.md). The owner deferred manual testing until later; implementation and focused automated checks continue without claiming physical acceptance. There is still no user-facing file converter.
