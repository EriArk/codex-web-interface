...
Release `d20c9d1` contains passes 1–3 and the owner-authority clarification. Built production image includes the updated Windows installer helpers; isolated image startup/private-socket/auth/public-assets checks and live owner Codex/GPT connectivity passed. The guarded `codex-web-member-d20c9d1.service` replaces the still-waiting `1455724` updater. At this checkpoint the engine remains `b273f5e` and maintenance is `waiting`; a fresh coordinated backup precedes the eventual idle switch. This is a queued release, not an installation claim. Registration remains disabled.
