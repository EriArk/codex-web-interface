...
- Retain normalized public native history privately in `results/gpt/history` across engine replacement. Snapshots are atomic, limited to 64 chats / 64 MiB, at most 8 MiB per chat and seven days old. They contain no raw native mappings or private tool/reasoning payloads. Transient read failures may serve a marked saved branch to paged history/Results; fresh reads used for mutation preconditions, source checks and search still fail honestly. Authentication and deletion never use this fallback, and confirmed deletion removes its saved snapshot.
- Display and completion observers share a recent native read for 15 seconds, including final-status reconciliation. Explicit native edit/version checks remain fresh. Public messages retain commentary/final phase and completion state so a commentary message cannot hide a completed outbox answer while final history is delayed. Native sends are never replayed by recovery.
- A cold browser can render the Hub snapshot without its own cache. Oversized outbox answers no longer delete every same-tab history snapshot. Initial-load Retry owns one local error; saved-history refresh uses one compact status and preserves the composer.
...
Engine/connector installation still requires genuine idle maintenance, including work in detached terminals. The owner's 2026-09-13 policy allows replacing verified idle shells; merely having a terminal open no longer blocks it. The independent gateway does not own these operations and cannot replace their execution engine during an active task. Compatible existing UI publication remains independent; prepared native functionality must not be described as installed before the engine and connector postchecks pass.
