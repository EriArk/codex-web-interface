...
## Implemented enrollment and upload controls

The first account is claimed only with a private single-use setup token, stored as a SHA-256 digest until consumption. The user sets their own password; Argon2id uses 64 MiB, three iterations and one lane. Passwords and raw session tokens are not stored. Secure/HttpOnly/SameSite=Strict cookies and exact-Origin/CSRF checks protect writes. Logout revokes event and Remote sockets. Static PWA assets contain no conversation data, and API responses are not cached.

Uploads require authentication and CSRF before body parsing. They are bounded to 25 MiB each, eight files and 64 MiB per message, with an initial 2 GiB storage quota. File identifiers and destinations are generated by the Hub. Original downloads use application/octet-stream and attachment disposition; only re-encoded JPEG previews are served inline. Raster processing has a pixel limit. Cross-thread attachment reuse and removal after send are rejected.

VNC is restricted to the trusted LAN path and the Hub source IP. Browser traffic is HTTPS/WSS. Native VNC does not provide end-to-end payload encryption across that LAN hop; use a separately configured encrypted LAN/Tailnet transport if the LAN threat model changes. Do not expose VNC or guacd publicly.
...
Windows attachment staging uses the system SFTP executable and existing SSH identity/host-key checks. It copies only an authenticated Hub-owned blob to a generated temporary path under the fixed CodexWeb attachment directory; verified length/hash and a bounded structured acknowledgement are required before native submission. Failed partial uploads receive bounded deletion of that exact temporary file. File bytes never enter a shell command or PowerShell console input. Remote diagnostics and private paths are not returned in user-facing errors.


## Implemented owner credential lifecycle

Schema 8 records a credential revision and one expiring SHA-256 recovery-token digest; it does not contain plaintext passwords or recovery links. Authenticated password change verifies the current password and rotates the cookie/CSRF pair after atomically revoking all previous sessions. Credential operations have exact-Origin/CSRF checks (except secret-bearing link redemption, which requires Origin) and a five-per-minute limit. A failed current-password check returns 403 without logging out a valid session.
