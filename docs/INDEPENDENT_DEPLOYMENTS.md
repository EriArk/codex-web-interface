...
`--enable-team-owner` is the bounded first-owner activation: require an empty team namespace and extra migration/image/rollback evidence, wait on the same live-work guard, then enable team routing with new registrations paused. The owner's existing password/session/private runtime and native configuration are retained. Before opening the gateway, `owner-team-check` verifies the original login/hash mapping, one-owner registry, closed registration and ready personal runtime. A pre-admission failure restores the previous configuration/database and preserves the failed registry under a separate private name. This script refuses already-enabled teams; their coordinated registry/profile recovery remains a separate admission gate.
