...
GPT uses the same isolated viewer for HTML fenced demos from the current native branch. Categories project assistant files and images from canonical history, independently of the chat's latest-20-message window. Uploads remain in the chat. Opening a demo uses an inline Preview tab, with optional expansion; switching categories retains its local state.

## MCP interactive requests

`mcpServer/elicitation/request` uses the existing authenticated pending-request channel. Standard MCP forms render typed strings, booleans, numbers/integers, single selections and multiple selections, with schema constraints validated again on the Hub. Accept, decline and cancel are distinct native actions. Pending requests survive browser reload; native resolution or a lost App Server connection expires the corresponding controls. Exact response retries share a receipt and cannot respond twice. Answer content is not added to chat, Results or the durable receipt body.

URL-mode requests show the service host and open an authenticated, no-store redirect in a separate window only after an explicit click. Signed URLs remain out of shared events and history. The owner explicitly confirms completion or cancels. Only HTTP(S) URLs without embedded credentials are accepted; the Hub does not fetch them.

Bounds are 40 fields, 100 choices per field and 8,000 characters per answer. Unknown schema constraints and extended OpenAI forms are visibly unsupported with decline/cancel controls; the extended-form capability is not negotiated. This does not claim support for arbitrary desktop dynamic tools. The implementation follows the installed Codex schema and the [App Server MCP request contract](https://learn.chatgpt.com/docs/app-server).
