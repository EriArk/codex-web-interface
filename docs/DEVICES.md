...
Power and network-mount forms confirm a named target and open a separate command terminal, never inject commands into a running shell/program. Linux uses ordinary sudo permissions; SMB requires the host's cifs-utils and NFS requires its mount helper. Windows uses PowerShell and SMB mappings in the SSH logon session; the same terminal stays open on the connected drive so it can be used immediately. An action can fail for OS permissions or missing helpers; its actual terminal output remains visible. No destructive action is exercised in automated or production smoke tests.
