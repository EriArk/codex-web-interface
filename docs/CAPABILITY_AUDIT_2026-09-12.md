...
## Native workspace follow-up — 2026-09-13

The next pass adds native scheduled-task management/creation requests, access to saved Canvas documents and version restore, and the installed desktop's extended resource/image forms. See [Native workspace](NATIVE_WORKSPACE.md) for access points, verified contracts and recovery rules. The live scheduled-task cycle was verified on one disposable future task, including the native 201/410 responses. Ordinary human reminders and chat drafts remain separate.

Full two-way voice was explicitly declined by the owner. Direct Canvas creation/editing remains unverified/unavailable in the current test environment; saved-document/version support is not represented as full Canvas parity. Event-driven schedule trigger editing still uses the original interface. The rejected desktop-tool inventory still provides no additional concrete host tool to implement. Distribution and the existing #6/#37 deferrals remain unchanged. Deployment status is reported separately from implementation and tests.
