...
## Engine updates after Team activation

`ops/linux/upgrade-engine.py` now supports an already-enabled Team. This is host tooling, not a new browser endpoint. Keep `engine_checkpoint.py` beside it in the exact verified release checkout. The proof additionally requires `teamCheckpoint`, `teamRollback`, `teamUpgradeAdmission` and `codexContinuityPreflight`; `--check` validates configuration/revisions and reads all-user activity without stopping services. First-owner activation keeps its separate, tested migration path.

After the existing all-user/terminal reservation confirms idle work, the updater closes the gateway and stops the engine. It also holds the GPT host lock against concurrent profile provisioning/recovery. It saves a cold checkpoint of Hub `data/`: registry, initialized personal databases, private uploads/results, shared assets and other managed files, together with exact configuration, deployment revision pair and current web pointer. Every initialized namespace, including disabled members, must exist; shared projects must retain one matching owner. Sources and copies are checksummed, SQLite integrity/foreign keys checked, and the exact restore copy rehearsed before the candidate starts. WAL contents are retained; SQLite shared-memory indexes are rebuilt. Checkpoints are private and never automatically pruned by this updater.

Running GPT profiles under each user's `gpt/` mount are deliberately excluded. Their original directories/inodes and the host lock remain in place through rollback; the original owner's separate GPT profile, native Windows credentials/history/repositories, SSH keys and external services are not replaced. Use the separate stopped-profile backup workflow for disaster recovery or profile relocation. This engine checkpoint is for the same installation and paths, not a portable complete-machine backup.

Before public admission, verify all saved Team access/content/binding columns, the original-owner mapping, private schema tables/passwords/native thread IDs and the unchanged configuration. New schema columns are allowed; lost access tables, changed account/profile bindings or changed ownership fail closed. The original schema, all private data and failed candidate writes remain recoverable. Do not weaken this check to make an unexplained migration pass; an intentional data transformation needs an explicit version-specific migration/rehearsal.

On a failure **before** public admission, stop the candidate and restore every managed namespace and the exact previous engine/web pair. Failed data is moved into a private `engine-failed-*` directory with a restore journal, never deleted. A restore interrupted by a host/storage failure leaves the service closed and the checkpoint/journal available for host recovery; inspect and complete it before restarting. Automatic restore preserves sessions and unknown receipts exactly because no replacement gateway has admitted new work. This differs from disaster recovery, which revokes sessions and blocks native replay.

Immediately before the candidate gateway starts, the updater persists `admitted.json`. After this boundary an error requires forward repair: the rollback helper refuses to overwrite storage, even if a later health/Doctor check fails. Never remove this marker merely to force an old database over newer user writes. Engine/schema updates still wait for active or unknown Codex/GPT work; this change does not turn a live task into an idle one.

Verification: `node --test tests/owner-team-upgrade.test.mjs tests/team-engine-upgrade.test.mjs`, plus the existing Team isolation, profile-backup and engine-socket tests on Linux. The Python fault suite exercises three account states, exact rollback, private/shared files, WAL, missing/corrupt namespaces, wrong bindings/native identities, protected profile inodes, the final activity reservation and pre-/post-admission failures. No native account is connected by these fixtures.
