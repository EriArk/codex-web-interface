...
Source-specific entry keeps its own account-local draft, edited references and pending send receipt: each notification batch/group and each exact Activity source is separate from the ordinary project draft. Returning to the same entry restores edits instead of reapplying its original URLs. The persistent native Intake conversation remains one per user/project; opening an entry still sends nothing. The two-user browser workflow checks notification close/reopen and verifies that the unrelated ordinary draft and sources remain unchanged.

Activity uses an explicit source-action row and equal GPT/Codex buttons, with reactions/discussion separated below. Intake uses compact named icon controls in its heading and symmetric phone gutters, preserving long-project-name access without crowding Close. [Layout rules](UI_LAYOUT_RULES.md) are mandatory for future changed windows. Chromium/WebKit checks include header separation, aligned touch controls and actual screenshots in all themes.
