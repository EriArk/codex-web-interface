...
for(const [engine,type] of [["chromium",chromium],["webkit",webkit]]){
 ...
 try{
  ...
  let active=0,known=true,restarts=0,operation=null,finish=false;
  ...
  await page.route("**/api/machines/*/desktop",async route=>{
   ...
   await route.fulfill({json:{available:true,running:true,activityKnown:known,activeTasks:active,operation}});
  });
  ...
  const open=async()=>{await page.getByRole("button",{name:"Настройки",exact:true}).tap();await page.locator(".settings-dialog[open]").waitFor();};
  ...
  for(const [theme,label] of [["organizer","Органайзер"],["crt-green","Зелёный терминал"],["hitech-2000s","Hi-Tech 2000s"]]){
   await dialog.locator(".theme-option."+theme).tap();await restart.scrollIntoViewIfNeeded();
  ...
  report.push({engine,confirmation:true,cancelHasNoEffect:true,oneRestart:true,reloadTracksOperation:true,activeBlocked:true,unknownBlocked:true,phoneAndTablet:true,allThemes:true,pageErrors:errors});
