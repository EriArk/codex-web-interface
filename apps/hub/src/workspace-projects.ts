...
export function workspaceProjects(sessions: Sessions, offset = 0): TaskProjectsPage {
  ...
  const saved = sessions.store.db
    .prepare(
      "SELECT scope FROM (SELECT scope,row_number() OVER (PARTITION BY scopeKey ORDER BY updatedAt DESC,id) AS n FROM (SELECT scope,scopeKey,updatedAt,id FROM workspace_tasks UNION ALL SELECT scope,scopeKey,updatedAt,id FROM workspace_notes UNION ALL SELECT scope,scopeKey,updatedAt,scopeKey AS id FROM project_cores UNION ALL SELECT scope,scopeKey,updatedAt,id FROM project_plans UNION ALL SELECT scope,scopeKey,createdAt AS updatedAt,id FROM project_reports) WHERE scope IS NOT NULL) WHERE n=1 LIMIT 10000",
    )
    .all();
