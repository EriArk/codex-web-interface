...
The installed canary permits one disposable chat only. One plain-text request passed through the Hub queue and the official native app, then canonical readback confirmed its exact UUID once and the expected answer. Reopening the Hub process and restarting the native container preserved that receipt, response and previous sandbox file hashes. This proves bounded existing-chat text sends, **not** new-chat/project/attachment queue parity, live-stream acceptance or a production provider switch.
