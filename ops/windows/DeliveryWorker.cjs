...
const read=async (file,max=8388608)=>{const s=await fs.lstat(file);if(!s.isFile()||s.isSymbolicLink()||s.size>max)throw Error('DELIVERY_REQUEST');return JSON.parse(await fs.readFile(file,'utf8'));};
...
async function worker(){
 ...
 while(Date.now()<deadline&&empty<8){
  ...
  for(const file of (await fs.readdir(root)).filter(n=>/^[a-f0-9-]{36}\.request\.json$/.test(n)).slice(0,12)){
   ...
   let result;
   try {const job=await read(path.join(root,file),4194304);if(!valid(job)||(job.request.op!=='github'&&Buffer.byteLength(JSON.stringify(job))>524288)||Date.now()-job.at>300000)throw Error('DELIVERY_REQUEST');result={ok:true,value:job.request.op==='github'?await (await import('./githubWorkProbe.js')).githubWorkProbe(job.root,job.request.request):await deliveryProbe(job.root,job.request)};}
...
async function request(){
 let source='';for await(const chunk of process.stdin){source+=chunk;if(Buffer.byteLength(source)>4194304)throw Error('DELIVERY_REQUEST');}
 const value=JSON.parse(source);if(!valid(value)||(value.request.op!=='github'&&Buffer.byteLength(source)>524288))throw Error('DELIVERY_REQUEST');
